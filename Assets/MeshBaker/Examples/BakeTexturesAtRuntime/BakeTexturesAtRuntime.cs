...
public class BakeTexturesAtRuntime : MonoBehaviour {
    ...
    public MB3_MeshCombinerSettings settings;
    ...
    public void Prepare()
    {
        CombineMeshAndTextures(target);
    }
    private void CombineMeshAndTextures(GameObject original)
    {
        var meshBaker = original.AddComponent<MB3_MeshBaker>();
        meshBaker.meshCombiner.settingsHolder = settings;

        var textureBaker = original.AddComponent<MB3_TextureBaker>();
        textureBaker.resultMaterial = material;
        textureBaker.objsToMesh = original.GetComponentsInChildren<MeshFilter>().Select(x => x.gameObject).ToList();

        BakeAtOnce();
        Debug.Log("Finish");
    }

    private void BakeAtOnce()
    {
        MB3_MeshBaker meshbaker = target.GetComponentInChildren<MB3_MeshBaker>();
        MB3_TextureBaker textureBaker = target.GetComponent<MB3_TextureBaker>();
    ...
    private void BakeOverCoroutine()
    {
        Debug.Log("Starting to bake textures on frame " + Time.frameCount);
        MB3_TextureBaker textureBaker = target.GetComponent<MB3_TextureBaker>();
        ...
        textureBaker.textureBakeResults = ScriptableObject.CreateInstance<MB2_TextureBakeResults>();
        //textureBaker.resultMaterial = BakerMaterial;
